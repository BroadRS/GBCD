...
def Z_ScoreNormalization(x, mu, sigma):
	...
	return x;
def show_accuracy(test,predictLabel,Label):
	Label = np.ravel(Label).tolist()
	predictLabel = predictLabel.tolist()
	count = 0
	for i in range(len(Label)):
		# print(str(Label[i])+'and'+str(test[(predictLabel[i])]))
		if Label[i] == [test[predictLabel[i]]]:
			count += 1
...
class broadNet(object):
	def __init__(self, map_num=10,enhance_num=10,DESIRED_ACC = 0.99, EPOCH = 10,STEP = 1, map_function='linear',enhance_function='linear',map_batchsize='auto',enh_batchsize = 'auto'):
		self.map_num = map_num    # 多少组mapping nodes
		self.enhance_num = enhance_num  # 多少组engance nodes
		self.map_batchsize = map_batchsize
		self.enh_batchsize = enh_batchsize
	...
	def fit(self, data, label):
		if self.map_batchsize == 'auto':
			self.map_batchsize = data.shape[1]
		if self.enh_batchsize == 'auto':
			self.enh_batchsize = data.shape[1]
		data = self.normalscaler.fit_transform(data)
		label = self.onehotencoder.fit_transform(np.mat(label).T)
		mappingdata = self.mapping_generator.generator_nodes(data, self.map_num, self.map_batchsize,self.map_function)
		enhancedata = self.enhance_generator.generator_nodes(mappingdata, self.enhance_num, self.enh_batchsize,self.enhance_function)
		...
		while accuracy < self.DESIRED_ACC and epoch_now < self.EPOCH:
			Y = self.addingenhance_predict(data, label, self.STEP, self.enh_batchsize)
			accuracy = self.accuracy(Y, label)
			epoch_now += 1
			print("enhencing {3}, number of mapping nodes {0}, number of enhence nodes {1}, accuracy {2}".format(len(self.mapping_generator.Wlist)*self.map_batchsize,
            																				                     len(self.enhance_generator.Wlist)*self.enh_batchsize,
                                                                                                                 round(accuracy,5),
                                                                                                                 epoch_now))
	...
	def weight_decode(self,Y_onehot,diff):
		Y = []
		for i in range(Y_onehot.shape[0]):
			lis = np.ravel(Y_onehot[i,:]).tolist()
			weight = 0
			...
			for j in range(len(lis)):
				total  +=(lis[j]-min(lis))/(max(lis)-min(lis))
			for j in range(len(lis)):
				weight += diff*(j+1)*(lis[j]-min(lis))/(max(lis)-min(lis))/total
			# print(((weight)))
			# print(round(weight))
			Y.append((weight))
		return np.array(Y)
	...
	def weightPredict(self, testdata,diff):
		#print(self.W.shape)
		testdata = self.normalscaler.transform(testdata)
		test_inputdata = self.transform(testdata)
		return self.weight_decode(test_inputdata.dot(self.W),diff)
